...
from dataprep import get_all_dataloaders
import diffusers
...
# GET Huggingface login key from environment variable
...
def parse_args(input_args=None):
    ...
    print(f"Loading Stable Diffusion Model {args.pretrained_model_name_or_path}")
    if args.dataset_name is None and args.root_image_dir is None:
        raise ValueError("Specify either `--dataset_name` or `--root_image_dir`")

    if args.dataset_name is not None and args.root_image_dir is not None:
        raise ValueError("Specify only one of `--dataset_name` or `--root_image_dir`")
...
def encode_prompt(
    ...
    for tokenizer, text_encoder in zip(clip_tokenizers, clip_text_encoders):
        prompt_embeds, pooled_prompt_embeds = _encode_prompt_with_clip(
            ...
            tokenizer=tokenizer,
            prompt=prompt,  # NOTE: Only taking 77 tokens
    ...
    # NOTE: Clip embedding is added to the end of the prompt embeddings
...
def main(args):
    ...
    train_dataloader, val_dataloader, test_dataloader = get_all_dataloaders(
        root_image_dir=args.root_image_dir,
        root_annot_dir=args.root_annot_dir,
    ...
    # Training Loop
    epoch=first_epoch
    #! NOTE: This is where the training loop starts.
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch_data in enumerate(train_dataloader):
            ...
            if (
                len(batch_data[0]) == 0
                or len(batch_data[1]) == 0
                or isinstance(batch_data[0], list)
            ):
                logger.warning("Skipping batch as it contains no data.")
                continue

            models_to_accumulate = [transformer]
            with accelerator.accumulate(models_to_accumulate):
                pixel_values = batch_data[0].to(dtype=vae.dtype)
                prompts = batch_data[1]
                ...
                if args.weighting_scheme == "sigma_sqrt":
                    ...
                    weighting = (
                        1
                        - u
                        - args.mode_scale * (torch.cos(math.pi * u / 2) ** 2 - 1 + u)
                    )
